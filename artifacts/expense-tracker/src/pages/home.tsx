...
import { z } from "zod";
import { PlusCircle, Trash2, Wallet, AlertTriangle, TrendingUp } from "lucide-react";
...
const CATEGORIES = [
  { key: "Food", label: "Food", emoji: "🍜", color: "indigo" },
  { key: "Transport", label: "Transport", emoji: "🚗", color: "sky" },
  { key: "Fun", label: "Fun", emoji: "🎉", color: "violet" },
  { key: "Belanja Bulanan", label: "Belanja Bulanan", emoji: "🛒", color: "emerald" },
  { key: "Biaya Tak Terduga", label: "Biaya Tak Terduga", emoji: "⚡", color: "amber" },
] as const;
...
const STORAGE_EXPENSES = "eviz-expenses-v2";
const STORAGE_BUDGETS = "eviz-budgets-v2";
...
export default function Home() {
  ...
  const getActual = useCallback(
    (catKey: CategoryKey) =>
      expenses.filter((e) => e.category === catKey).reduce((s, e) => s + e.amount, 0),
    [expenses]
  );

  const totalBudgeted = CATEGORIES.reduce((s, c) => s + (budgets[c.key] || 0), 0);
              ...
              >
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-slate-500">
                    <span>Terpakai</span>
                    <span className={isOver ? "text-red-500 font-semibold" : colors.text}>
                      {formatRp(actual)}
                    </span>
                  </div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          placeholder="Contoh: Nasi goreng, Bensin, dll."
